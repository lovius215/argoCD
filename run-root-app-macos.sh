...
echo "============================================================"
echo "🚀 [macOS] 部署 / 執行 Argo CD Root Application (App of Apps)"
echo "============================================================"

# 1. 檢查 kubectl 是否存在
if ! command -v kubectl &> /dev/null; then
  echo "❌ 錯誤: 未找到 kubectl 指令，請先安裝 kubectl (macOS)。"
  echo "   可使用 brew 安裝: brew install kubectl"
  exit 1
fi
...
if ! kubectl cluster-info &> /dev/null; then
  echo "❌ 錯誤: 無法連線至 Kubernetes 叢集，請確認 Docker Desktop / OrbStack / kind / minikube 是否已啟動。"
  exit 1
fi
echo "✅ K8s 叢集連線正常"
